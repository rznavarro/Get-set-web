...
export function Dashboard({ formData }: DashboardProps) {
  ...
  const handleRegenerateSummary = async () => {
    ...
    try {
      const investorId = (formData as any).investorId || localStorage.getItem('portfolio_ceo_investor_id');

      // Prepare the current dashboard data to send to webhook
      const dashboardData = {
        action: 'regenerate_executive_summary_with_current_data',
        timestamp: new Date().toISOString(),
        formData: formData,
        investorId: investorId,
        currentMetrics: data.metrics,
        currentOpportunities: data.analysis.critical_actions,
        currentQuickActions: data.next_30_days
      };
      ...
      // Update the data with the new executive summary
      ...
      // Update critical actions and next 30 days if provided
      if (result.critical_actions) {
        updatedData.analysis.critical_actions = result.critical_actions;
      }
      if (result.next_30_days) {
        updatedData.next_30_days = result.next_30_days;
      }

      setData(updatedData);

      // Update localStorage if we have formData
      const updatedFormData = {
        ...formData,
        aiGeneratedSummary: updatedData.analysis.executive_summary
      };
      localStorage.setItem('portfolio_ceo_form_data', JSON.stringify(updatedFormData));

      // Save to Supabase if we have investor ID
      if (investorId) {
        await import('../lib/api').then(({ saveAnalysisData }) => {
          saveAnalysisData(investorId, {
            analysis: updatedData.analysis,
            metrics: updatedData.metrics,
            next_30_days: updatedData.next_30_days
          });
        });
      }
